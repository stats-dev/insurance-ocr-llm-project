...
# --- 게스트 모드: 데모만 표시 ---
    ...
    st.divider()
    st.warning("🔒 **게스트 모드입니다.** API 사용량 제한을 위해 파일 업로드는 관리자에게만 허용됩니다.")
    st.info("아래는 AI 에이전트가 어떻게 동작하는지 보여주는 **예시(Demo)**입니다.")
...
# 하단 설명
st.divider()
st.info("💡 **Tip:** 이 프로젝트는 OCR의 한계(오인식)를 LLM의 '문맥 이해 능력'으로 극복하는 **Data-Centric AI** 접근 방식을 보여줍니다.")
